...
import time
from dataClean import denoiseImg_isolation

'''
This copy of code is a comprehensive gan (cnn or fcn for discriminator) for segmentation, 
part of the work have been submitted to TNNLS. Actually, when I develop this program, I donot have a strong GPU, so that I implment it in
a 2D way. Now, I have a GPU cluster and I have already implemented a 3D version.
The networks:
1. weighted softmax loss, dice loss, focal loss for segmentation
2. stochastic residual module
3. dilation in the smallest resolution
4. local conv
5. contour-aware loss
6. regression the intensities
8. sample attention mechanism
...
I will upload the whole copy of codes to my github(https://github.com/ginobilinie/Strainet) when it is accpeted.
...
parser.add_argument("--isDiceLoss", action="store_true", help="is Dice Loss used?", default=False)
...
parser.add_argument("--isViewExpansion", action="store_true", help="is view expanded (dilation used)?", default=True)
parser.add_argument("--isLocalUsed", action="store_true", help="is local used?", default=False)
...
parser.add_argument("--prefixModelName", default="/home/dongnie/Desktop/myPyTorch/pytorch-SRResNet/Segmentor_wce_wdice_contour_lrdcr_1127_", type=str, help="prefix of the to-be-saved model name")
...
##########configs######## 
    ...
    if opt.isSegReg:
        ...
        if opt.isLocalUsed:
            netG = ResSegNet_v1(isRandomConnection=opt.isResidualEnhancement,isSmallDilation=opt.isViewExpansion, isSpatialDropOut=opt.isSpatialDropOut,dropoutRate=opt.dropoutRate)
        else:
...
#             print 'running loss is ',running_loss
            ...
            if opt.isDiceLoss and opt.isSoftmaxLoss and opt.isAdLoss and opt.isSegReg:
                ...
                print 'loss G_G is %.2f.'%lossG_G.data[0]
            if opt.isContourLoss:
        ...
        if iter%opt.showTestPerformanceEvery==0: #test one subject  
            ...
            mr_test_itk=sitk.ReadImage(os.path.join(path_test,'img50_nocrop.nii.gz')) #this is one of the subjects in valdiation set
...
#                 sitk.WriteImage(volGT,'volGT'+'.nii.gz')
            ...
            matOut,_ = testOneSubject(matFA,matGT,4,[5,168,112],[1,168,112],[1,8,8],netG,opt.prefixModelName+'%d.pt'%iter)
