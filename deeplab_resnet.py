...
#         self.interp3 = nn.Upsample(size = (outS(input_size),   outS(input_size)),mode='bilinear')
        self.interp1 = nn.Upsample(size = (int(input_size*0.75),  int(input_size*0.75) ),mode='bilinear')
        self.interp2 = nn.Upsample(size = (int(input_size*0.5),   int(input_size*0.5) ),mode='bilinear')
        self.interp3 = nn.Upsample(size = (outS(input_size),   outS(input_size)),mode='bilinear')
        out = []
        x2 = self.interp1(x)
        x3 = self.interp2(x)
...
#         print 'x3.size: ',x3.size()
        ...
        #problem comes from out

        x2Out_interp = out[1]
        x3Out_interp = self.interp3(out[2])
#         print 'x2Out size: ',x2Out_interp.size(),'x3Out size: ',x3Out_interp.size(),'out[0] size: ',out[0].size()
