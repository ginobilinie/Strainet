...
#         print 'come xavier'
        ...
        m.bias.data.fill_(0)
'''
initialize the conv2D (transposeCconv2D) with gaussian distribution
'''
...
'''
def testOneSubject(MR_image, CT_GT, NumOfClass, MR_patch_sz, CT_patch_sz, step, netG, modelPath, resType):
    eps=1e-5
...
#                 sitk.WriteImage(volPre,'volPre_{}'.format(cnt)+'.nii.gz')
                if resType==0 or resType==2:
                    tempprobmat = softmax2d(tempprobmat)
